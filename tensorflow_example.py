...
for filename in filenames:
    ...
    mel_spectrogram = librosa.feature.melspectrogram(S=np.abs(spectrogram)**2,
                                                     n_fft=1024,
                                                     hop_length=512,
                                                     n_mels=128)
...
train_log_mel_spectrograms = np.expand_dims(train_log_mel_spectrograms,
                                            axis=-1)
train_delta_log_mel_spectrograms = np.array(train_delta_log_mel_spectrograms)
train_delta_log_mel_spectrograms = np.expand_dims(train_delta_log_mel_spectrograms,
                                                  axis=-1)
train_target_numbers = np.array(train_target_numbers)
val_log_mel_spectrograms = np.array(val_log_mel_spectrograms)
val_log_mel_spectrograms = np.expand_dims(val_log_mel_spectrograms,
                                          axis=-1)
val_delta_log_mel_spectrograms = np.array(val_delta_log_mel_spectrograms)
val_delta_log_mel_spectrograms = np.expand_dims(val_delta_log_mel_spectrograms,
                                                axis=-1)
val_target_numbers = np.array(val_target_numbers)
test_log_mel_spectrograms = np.array(test_log_mel_spectrograms)
test_log_mel_spectrograms = np.expand_dims(test_log_mel_spectrograms,
                                           axis=-1)
test_delta_log_mel_spectrograms = np.array(test_delta_log_mel_spectrograms)
test_delta_log_mel_spectrograms = np.expand_dims(test_delta_log_mel_spectrograms,
                                                 axis=-1)
...
model = tf.keras.Sequential([tf.keras.layers.Conv2D(filters=32,
                             ...
                             tf.keras.layers.Conv2D(filters=128,
                                                    ...
                                                    padding="same"),
                             tf.keras.layers.BatchNormalization(axis=-1),
                             tf.keras.layers.MaxPool2D(pool_size=(4, 2),
                                                       padding="same"),
...
model.fit(np.concatenate((train_log_mel_spectrograms,
                          train_delta_log_mel_spectrograms),
                         axis=-1),
          train_target_numbers,
          epochs=125,
...
for threshold in np.arange(0.1, 0.99, 0.05):
    _, val_acc = model.evaluate(np.concatenate((val_log_mel_spectrograms,
                                                val_delta_log_mel_spectrograms),
                                               axis=-1),
                                val_target_numbers,
                                verbose=0)
...
_, test_acc = model.evaluate(np.concatenate((test_log_mel_spectrograms,
                                             test_delta_log_mel_spectrograms),
                                            axis=-1),
                             test_target_numbers,
                             verbose=0)
